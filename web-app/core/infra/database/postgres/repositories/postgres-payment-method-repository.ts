...
export class PostgresPaymentMethodRepository implements PaymentMethodRepository {
  public async add(payload: PaymentMethodRepository.AddPayload): Promise<PaymentMethod> {
  ...
  public async update(
    ...
    try {
      ...
      const isFeeChanged = rowsLastPaymentMethodFees.some((row) => !fee.isEqual(row.fee))
      ...
      const valuesChangeFee = isFeeChanged ? [paymentMethodId, fee.int] : [paymentMethodId]
  ...
  public async existsIds(ids: { paymentMethodId: number }[]): Promise<boolean> {
    const paymentMethodIds: string = ids.map(({ paymentMethodId }) => paymentMethodId).join(", ")
  ...
  public async getByCompanyId(companyId: number, offset: number, limit: number) {
    const { rows } = await PostgresService.query<{
      id: number
      name: string
      fee: number
